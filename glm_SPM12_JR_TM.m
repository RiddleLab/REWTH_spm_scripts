function [varargout] = glm_SPM12_JR_TM(RESULTS_DIR, epiFiles, glmInfo)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   CREDITS
%
%       Justin Riddle
...
%       This version was modified by TM on 5-10-26
...
if (~isempty(ALL_DES_MAT) || ~isempty(extraContrasts))
    ...
    for epiIdx = 1:numEPIs
        ...
        for regressIdx = 1:numRegressors(epiIdx) % TM modified on 5-10-26
            orderIdx = orderIdx + 1;
