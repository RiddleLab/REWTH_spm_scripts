...
%% Step 1: deoblique the subject's anatomical scan
deoblique_structFile = [SUB_ROI 'deoblique_' subSpace_structFilename];
if exist(deoblique_structFile,'file')~=2

    % Create a copy that will serve as the reference grid
    subStruct_V = spm_vol(subSpace_structFile);
    
    % Force the affine to be cardinal (orthogonal)
    subStruct_AffineM = subStruct_V.mat;
    
    % Calculate voxel size
    voxsize = sqrt(sum(subStruct_AffineM(1:3,1:3).^2));
    
    % Generate a new affine matrix
    M_new = eye(4);
    M_new(1,1) = voxsize(1);
    M_new(2,2) = voxsize(2);
    M_new(3,3) = voxsize(3);
    
    % Preserve approximate image center
    center_vox = (subStruct_V.dim + 1)/2;
    center_mm  = subStruct_V.mat * [center_vox 1]';
    
    M_new(1:3,4) = center_mm(1:3) - M_new(1:3,1:3)*center_vox';
    
    subStruct_V_new = subStruct_V;
    subStruct_V_new.mat = M_new;
    subStruct_V_new.fname = deoblique_structFile;
    subStruct_R = spm_read_vols(subStruct_V);
    spm_write_vol(subStruct_V_new,subStruct_R)
    
    % Reslice onto the new grid
    flags = struct('interp',4,'mask',0,'mean',0,'which',1,'wrap',[0 0 0]);
    
    spm_reslice({deoblique_structFile},flags);
end


%% Step 2: Manually type in the rmPFC coodinates in MNI space
...
if exist(bNorm_rmPFC_roiFile,'file')~=2
    ...
    if run_rmPFCcreation
        ...
        %% Step 3: Normalize structural scan and then back-normalize the rmPFC ROI
        % files for structural get written into the STRUCT_DIR 'ROIs/'
        % epis go into reslice dir
        [backNormROIfiles] = backNormROI_REWTH(deoblique_structFile,...
            deoblique_structFile,{mni_rmPFC_roiFile},'bNorm_',SUB_ROI);
        movefile(backNormROIfiles{1},bNorm_rmPFC_roiFile);

    end
end

%% Step 4: check that the MNI subgenual cingulate exists
...
%% Step 5: Segment the MNI T1w before creating a nuissance mask
numTissues = 5;
allTissuesExist = 1;
...
%% Step 6: Create a nuissance mask for this participant in MNI space
...
if run_dlPFCcreation
    ...
    % epis go into reslice dir
    [backNormROIfiles] = backNormROI_REWTH(deoblique_structFile,...
        deoblique_structFile,{mni_dlPFC_roiFile},'bNorm_',SUB_ROI);
    movefile(backNormROIfiles{1},bNorm_dlPFC_roiFile);

    %% Step 10: run seed-based connectivity from the dlPFC ROI
